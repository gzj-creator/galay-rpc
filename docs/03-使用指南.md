...
| 选项 | 默认值 | 说明 |
|------|--------|------|
| BUILD_TESTING | ON | 构建测试（标准 CTest 开关） |
| BUILD_BENCHMARKS | ON | 构建压测 |
| BUILD_EXAMPLES | ON | 构建示例 |
| BUILD_MODULE_EXAMPLES | ON | 构建 C++23 `import` 示例（需 Ninja/VS 生成器） |
| GALAY_RPC_INSTALL_MODULE_INTERFACE | ON | 安装 `galay_rpc.cppm` 等模块接口文件；关闭后仅安装头文件公开面 |
| DISABLE_IOURING | ON | Linux 下禁用 `io_uring`，固定使用 `epoll` |

> `BUILD_MODULE_EXAMPLES` 需要 CMake `>= 3.28` 且使用 `Ninja`/`Visual Studio` 生成器。  
> `Unix Makefiles` 下会自动关闭该选项。
> `galay-rpc::galay-rpc` 是头文件 `INTERFACE` target；当前仓库没有共享库 / 静态库切换选项。
> `DISABLE_IOURING=OFF` 时，Linux 会尝试探测 `liburing` 并切换到 `io_uring`；未找到时自动回退到 `epoll`。
> `GALAY_RPC_INSTALL_MODULE_INTERFACE=OFF` 时，`install` 结果不会包含 `.cppm`；开启时仅在模块 target 可用时导出 `galay-rpc::galay-rpc-modules`。
...
int main() {
    ...
    runtime.getNextIOScheduler()->spawn(task);
...
include 版 Echo / Stream 链路分别由 `EchoExampleSmokeTest` 与 `StreamExampleSmokeTest` 自动验证。

当 `BUILD_MODULE_EXAMPLES=ON` 且生成器支持模块时，Echo import 链路由 `EchoImportExampleSmokeTest` 自动验证，真实 Stream import 链路由 `StreamImportExampleSmokeTest` 自动验证。
...
config.io_scheduler_count = 4;        // IO 调度器数量（显式值；自动请用 GALAY_RUNTIME_SCHEDULER_COUNT_AUTO）
...
```cpp
// 字符串
if (auto __await_result = co_await client.call("Service", "method", "Hello, World!"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
if (auto __await_result = co_await client.call("Service", "add", data, 8); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}

// 无 payload
if (auto __await_result = co_await client.call("Service", "ping"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
Coroutine asyncDiscovery() {
  ...
  ep.service_name = "MyService";
    ...
    // 异步注册
    if (auto reg = co_await registry.registerServiceAsync(ep).wait(); !reg) {
        std::cerr << "注册 await 失败\n";
        co_return;
    }
    ...
    // 异步发现
    if (auto discover = co_await registry.discoverServiceAsync("MyService").wait(); !discover) {
        std::cerr << "发现 await 失败\n";
        co_return;
    }
    auto endpoints = registry.lastEndpoints();

    // 异步注销
    if (auto deregister = co_await registry.deregisterServiceAsync(ep).wait(); !deregister) {
        std::cerr << "注销 await 失败\n";
    }
}
```
...
实现 `ServiceRegistry` concept 即可接入 etcd、consul 等。下面的代码块只展示接口契约，不代表仓库内置了 `EtcdServiceRegistry` 实现：
