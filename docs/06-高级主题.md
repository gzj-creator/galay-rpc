...
> 本页混合两类内容：
>
> - **源码已支持 / 可验证**：能够在公开头、`examples/`、`test/` 或 `benchmark/` 中找到依据
> - **说明性集成模式**：用于展示业务层或外部生态集成思路，不代表仓库内置了对应 target、第三方依赖或可直接编译的适配器
>
> 其中 `服务发现集成`、`熔断器`、`连接池`、`限流`、`监控`、`HTTP/JSON 集成` 默认按“说明性模式”理解，除非文中明确指向了仓库中的真实文件
...
- `RoundRobinSelector` / `RandomSelector` / `WeightedRoundRobinSelector` / `WeightedRandomSelector`

如果要对接 etcd、Consul 或其他注册中心，适配器必须满足 `ServiceRegistry` concept，也就是实现以下接口：
...
public:
    ...
    std::expected<std::vector<ServiceEndpoint>, DiscoveryError> discoverService(
        const std::string& service_name
    );
    std::expected<void, DiscoveryError> watchService(
        const std::string& service_name,
...
static_assert(ServiceRegistry<ExternalRegistryAdapter>);
```

适配时请注意：

- `ServiceEndpoint` 只有 `host`、`port`、`service_name`、`instance_id`、`weight` 五个字段；仓库 **没有** 提供 `ServiceEndpoint::fromJson(...)`
- 如果外部系统使用 JSON、KV 或 HTTP 响应，需要在你自己的适配器里显式完成序列化/反序列化
- `discoverService(...)` 必须返回 `std::vector<ServiceEndpoint>`；不要在文档或实现里直接返回未定义变量
- `watchService(...)` 的职责是把外部事件转换成 `ServiceEvent{ServiceEventType, ServiceEndpoint}` 再回调给调用方
- 仓库没有内置 etcd / Consul SDK，也没有提供可直接编译的第三方适配 target

### etcd / Consul 集成建议

- etcd：把 key 设计为 `/<prefix>/<service_name>/<instance_id>`，value 由你的适配器自行编码为 `ServiceEndpoint`
- Consul：把 HTTP API 返回结果转换成 `ServiceEndpoint` 列表，再交给 `ServiceDiscoveryClient`
- 两者都建议把网络错误、反序列化错误和 watch 中断统一映射为 `DiscoveryError`
...
Coroutine streamWithWindow(RpcClient& client) {
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        if (auto __await_result = co_await windowed_writer.sendData(data); !__await_result) {
            // 错误处理：记录日志、重试或提前返回
        }
    }
...
// 方案 1：自动检测（推荐）
config.io_scheduler_count = GALAY_RUNTIME_SCHEDULER_COUNT_AUTO;  // 自动，0 会禁用 IO scheduler
...
if (auto __await_result = co_await client.call("Service", "method", data); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
size_t len = 5;
if (auto __await_result = co_await client.call("Service", "method", data, len); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
public:
    ...
    Coroutine method(RpcContext& ctx) {
        ...
        // 记录请求（示例：可替换为任意业务日志框架）
        std::cout << "Request: service=" << req.serviceName()
        ...
        // 记录响应（示例：可替换为任意业务日志框架）
        std::cout << "Response: service=" << req.serviceName()
                  << ", method=" << req.methodName()
