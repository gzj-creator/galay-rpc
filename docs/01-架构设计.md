...
> 当前仓库的单一口径是：Linux 使用 `epoll`（默认）或 `io_uring`（`DISABLE_IOURING=OFF` 且找到 `liburing`），macOS 使用 `kqueue`；Windows 仅在 `cmake/option.cmake` 中保留实验性 `IOCP` 分支，FreeBSD/其他平台在本仓库内没有独立后端分支与验证承诺。
...
| 文件 | 职责 |
|------|------|
| `rpc_base.h` | 基础常量定义：魔数、版本号、消息类型枚举、错误码枚举 |
| `rpc_message.h` | 消息类定义：`RpcHeader`、`RpcRequest`、`RpcResponse` 的序列化/反序列化 |
| `rpc_codec.h` | 编解码器：提供静态方法解码完整消息，计算消息长度 |
| `rpc_error.h` | 错误类：封装错误码和错误信息 |
...
| 文件 | 职责 |
|------|------|
| `rpc_conn.h` | 连接封装：RingBuffer + readv/writev 高效 IO，提供 Reader/Writer 抽象 |
| `rpc_service.h` | 服务定义：`RpcService` 基类、方法注册、`RpcContext` 上下文 |
| `rpc_server.h` | 服务器：accept 循环、连接分发、请求处理、内置 Runtime |
| `rpc_client.h` | 客户端：连接管理、RPC 调用状态机、超时支持 |
| `rpc_stream.h` | 双向流：流初始化、数据传输、流结束/取消 |
| `rpc_discovery.h` | 服务发现：Concept 约束、本地/异步注册中心、负载均衡 |
...
galay-rpc (header-only)
    ...
    │       │
    │       ├── Linux: epoll（默认）/ io_uring（显式开启且检测到 liburing）
    │       ├── macOS: kqueue
    │       └── Windows: IOCP 配置分支（实验性，仓库未做原生 Windows 验证）
```
