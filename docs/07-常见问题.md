...
2. 服务名是否正确（区分大小写）：
   ...
   // 客户端
   if (auto call_res = co_await client.call("EchoService", "echo", "data"); !call_res) {  // 必须匹配
       std::cerr << "调用失败\n";
   }
   ```
...
if (auto __await_result = co_await client.call("Service", "method", &user, sizeof(user)); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
1. 是否先建立了普通连接：
   ```cpp
   if (auto conn = co_await client.connect("127.0.0.1", 9000); !conn) {  // 必须先连接
       std::cerr << "连接失败\n";
   }
   ```
...
**A:** `galay-rpc` 内部不依赖日志库。建议在业务层自行接入日志框架，并在调用返回值上打印错误细节：

```cpp
auto result = co_await client.call("Service", "method", "data");
if (!result) {
    std::cerr << "transport error: " << result.error().toString() << "\n";
}
```
